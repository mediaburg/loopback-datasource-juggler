...
Memory.prototype.connect = function(callback) {
  var self = this;
  if (this.connected) {
    // It's already connected or being connected
    // Do NOT run loadFromFile again as it will override the cache
  ...
  if (this.isTransaction) {
    ...
    this.loadFromFile(function(err) {
      if (!err) self.connected = true;
      self.connecting = false;
      // Invoke all pending callbacks
      async.each(self.pendingConnectCallbacks, function(cb, done) {
...
Memory.prototype.setupFileQueue = function() {
  ...
  if (!this.fileQueue) {
    ...
    this.fileQueue = async.queue(function(task, cb) {
      ...
      if (task.operation === 'write') {
        ...
        fs.writeFile(file, data, function(err) {
          ...
          cb(err);
          callback && callback(err, task.data);
        });
...
Memory.prototype.loadFromFile = function(callback) {
  ...
  if (self.settings.file) {
    debug('Queueing read %s', self.settings.file);
    ...
    data = data || '{}';
    self.parseAndLoad(data, callback);
  } else {
...
Memory.prototype.saveToFile = function(result, callback) {
  ...
  var localStorage = hasLocalStorage && this.settings.localStorage;
  if (file) {
    debug('Queueing write %s', self.settings.file);
    // Enqueue the write
    self.setupFileQueue().push({
      operation: 'write',
      data: result,
      callback: callback,
    });
